...
public:
	...
	__declspec(dllexport) DatabaseHandler(const std::string& databaseName, const std::string& databaseHost,
					const  std::string& databaseUser, const std::string& databasePassword);
	__declspec(dllexport) std::vector<std::vector<std::string>> GetTable(const std::string& tableName);
	__declspec(dllexport) std::vector<std::vector<std::string>> GetTableFromCommand(const std::string& command);
